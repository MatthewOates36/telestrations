...
if (!fs.existsSync(`data/users-${port}.json`)) {
    fs.writeFileSync(`data/users-${port}.json`, '{}')
}

if (!fs.existsSync(`data/gamedata-${port}.json`)) {
    fs.writeFileSync(`data/gamedata-${port}.json`, '{}')
}
...
const userHandler = new UserHandler(__dirname + `/data/users-${port}.json`)
const userTracker = new UserTracker(5)
const telestrationsHandler = new TelestrationsHandler(__dirname + `/data/gamedata-${port}.json`)
        ...
        case GameMode.SHOWING_BEST:
            if (lastGameMode !== GameMode.SHOWING_BEST || allUsersDone()) {
                ...
                if (bestTelestrations === undefined) {
                    let telestrations = telestrationsHandler.getTelestrations()
